...
class SimidProtocol {
    ...
    if (EventsThatRequireResponse.includes(messageType)) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (typeof console !== 'undefined' && console.log) {
          console.log('[SIMID Protocol] Send', message);
        }
  ...
  receiveMessage(event) {
    ...
    try {
      ...
      // If the message is not in JSON format, it is not a SIMID message and can be ignored.
      return;
    }
    if (!data) {
    ...
    if (typeof console !== 'undefined' && console.log) {
      console.log('[SIMID Protocol] Receive', data);
    }
